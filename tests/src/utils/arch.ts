...
export async function getConstantineConnection(mnemonic?: string) : Promise<[ArchwayClient, SigningArchwayClient]> {
  ...
  if (mnemonic) {
    ...
    let opt: SigningCosmWasmClientOptions = {};
...
export async function getGenesisWallets(): Promise<DirectSecp256k1HdWallet[]> {
    ...
    wallets[0] = await DirectSecp256k1HdWallet.fromMnemonic("grant rice replace explain federal release fix clever romance raise often wild taxi quarter soccer fiber love must tape steak together observe swap guitar");
    wallets[1] = await DirectSecp256k1HdWallet.fromMnemonic("jelly shadow frog dirt dragon use armed praise universe win jungle close inmate rain oil canvas beauty pioneer chef soccer icon dizzy thunder meadow");
    wallets[1] = await DirectSecp256k1HdWallet.fromMnemonic("chair love bleak wonder skirt permit say assist aunt credit roast size obtain minute throw sand usual age smart exact enough room shadow charge");
    wallets[1] = await DirectSecp256k1HdWallet.fromMnemonic("word twist toast cloth movie predict advance crumble escape whale sail such angry muffin balcony keen move employ cook valve hurt glimpse breeze brick");
    return wallets;
}
...
export async function requestFaucetTokensConstantine(address: string, coin: string): Promise<boolean> {
  let response = await fetch("https://faucet.constantine-2.archway.tech/", { method: "POST", body: {
    address,
    coins: [
      coin
    ]
